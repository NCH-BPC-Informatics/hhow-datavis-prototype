...
ExcellentExport = (function() {
    var version = "1.3";
    var uri = { excel: 'data:application/vnd.ms-excel;base64,', csv: 'data:application/csv;base64,', r: 'data:application/octet-stream;base64,' };
    // var uri = {excel: 'data:application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;base64,', csv: 'data:application/csv;base64,'};
    ...
    var tableToCSV = function (table) {
        ...
        for (var i = 0, row; row = table.rows[i]; i++) {
            ...
            // ESK HACK!  Skip rows that include our groupping text ;-)
            // Eventually this problem will go away because we'll build this dynamically from the JSON data
            // but for now we filter out these rows from the HTML table
            if (row.innerHTML.indexOf("Candidate Subset") < 0) {
                data = data + "\r\n";
            }
        }
        return data;
    ...
    var tableToRScript = function (table) {
        var data = "";
        data += "candidate_specimens <- read.csv(header = TRUE, text = \"\r\n";
        for (var i = 0, row; row = table.rows[i]; i++) {
            for (var j = 0, col; col = row.cells[j]; j++) {
                data = data + (j ? ',' : '') + fixCSVField(col.innerHTML);
            }
    ...
    var ee = {
        ...
        /** @expose */
        r: function(anchor, table) {
            table = get(table);
            var rScript = tableToRScript(table);
            var hrefvalue = uri.csv + base64(rScript);
            anchor.href = hrefvalue;
            return true;
        }
    };
